...
const PAYROLL_STATUS_COLORS: Record<string, string> = {
  ...
  PROCESSING: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-300 dark:border-amber-800",
...
export function EmployeeSalaryTab({ employeeId }: EmployeeSalaryTabProps) {
  ...
  const deductions = structure
    ? structure.pfEmployee + structure.esi + structure.tds
    : 0
        ...
        <div className="flex items-center justify-between">
          ...
          {ytdPaid > 0 && (
            <p className="text-xs text-muted-foreground">
              YTD paid:{" "}
              <span className="font-semibold text-foreground">{fmt(ytdPaid)}</span>
            </p>
          )}
        </div>
...
function SummaryCard({
  ...
  const valueColor = accent === "green" ? "text-emerald-600 dark:text-emerald-400" : "text-foreground"
  return (
    <div className="px-4 py-3">
      ...
      <p className={cn("mt-1 tabular-nums", isText ? "text-sm font-medium" : "text-lg font-bold", valueColor)}>
        {value}
      </p>
    </div>
  )
