...
export const PATCH = withSession(async (req: NextRequest, ctx: { params: Promise<{ id: string; messageId: string }> }, session: Session) => {
  try {
    const { messageId } = await ctx.params
    const msg = await db.projectMessage.findUnique({ where: { id: messageId }, select: { id: true, authorId: true } })
...
export const DELETE = withSession(async (_req: NextRequest, ctx: { params: Promise<{ id: string; messageId: string }> }, session: Session) => {
  try {
    const { messageId } = await ctx.params
    const msg = await db.projectMessage.findUnique({ where: { id: messageId }, select: { id: true, authorId: true } })
    if (!msg) return NextResponse.json({ error: "Message not found" }, { status: 404 })
