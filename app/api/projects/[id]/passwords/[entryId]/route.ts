...
export const GET = withSession(async (_req: NextRequest, ctx: { params: Promise<{ entryId: string }> }, _session: Session) => {
  try {
...
export const PATCH = withSession(async (req: NextRequest, ctx: { params: Promise<{ entryId: string }> }, session: Session) => {
  try {
    const { entryId } = await ctx.params
    const entry = await db.projectPasswordEntry.findUnique({ where: { id: entryId }, select: { id: true, createdById: true } })
    if (!entry) return NextResponse.json({ error: "Not found" }, { status: 404 })
...
export const DELETE = withSession(async (_req: NextRequest, ctx: { params: Promise<{ entryId: string }> }, session: Session) => {
  try {
    const { entryId } = await ctx.params
    const entry = await db.projectPasswordEntry.findUnique({ where: { id: entryId }, select: { id: true, createdById: true } })
    if (!entry) return NextResponse.json({ error: "Not found" }, { status: 404 })
