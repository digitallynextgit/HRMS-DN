...
export const PATCH = withSession(async (req: NextRequest, ctx: { params: Promise<{ itemId: string }> }, _session: Session) => {
  try {
...
export const DELETE = withSession(async (_req: NextRequest, ctx: { params: Promise<{ itemId: string }> }, _session: Session) => {
  try {
    const { itemId } = await ctx.params
